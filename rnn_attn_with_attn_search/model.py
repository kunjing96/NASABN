...
class RNNModel(nn.Module):
    ...
    def __init__(self, ntoken, ninp, nhid, nhidlast, dropout=0.5, dropouth=0.5, dropoutx=0.5, dropouti=0.5, dropoute=0.1, dropoutatt=0.0, cell_cls=DARTSCell, genotype=None):
        ...
        if genotype is not None:
            if genotype.attention == 'att':
                self.attn = Attention()
            elif genotype.attention == 'multiatt':
                self.attn = MultiHeadAttn(8, nhid, nhid, dropout=dropout, dropatt=dropoutatt)
        else:
            self.attn = torch.nn.ModuleList([Attention(), MultiHeadAttn(8, nhid, nhid, dropout=dropout, dropatt=dropoutatt)])
    ...
    def forward(self, input, hidden, return_h=False):
        ...
        raw_output = self.attn(raw_output)
        raw_outputs.append(raw_output)
