...
from genotypes import PRIMITIVES, STEPS, CONCAT, Genotype, ATTENTIONS
...
import numpy as np
from attention import Attention, MultiHeadAttn
...
class RNNModelSearch(RNNModel):
    ...
    def __init__(self, *args):
        ...
        #self.attn = [Attention(), MultiHeadAttn(8, self.nhid, self.nhid, dropout=self.dropout, dropatt=self.dropoutatt)]
    ...
    def _initialize_arch_parameters(self):
      ...
      weights_att_data = torch.randn(2).mul_(1e-3)
      self.weights_att = Variable(weights_att_data.cuda(), requires_grad=True)

      self._arch_parameters = [self.weights, self.weights_att]
    ...
    def forward(self, input, hidden, return_h=False):
        ...
        #raw_output = self.attn[0](raw_output)
        probs = F.softmax(self.weights_att, dim=-1)
        raw_output = sum([self.attn[i](raw_output) * probs[i,None,None] for i in range(len(self.attn))])
        raw_outputs.append(raw_output)
    ...
    def genotype(self):
      ...
      att = ATTENTIONS[np.argmax(F.softmax(self.weights_att, dim=-1).data.cpu().numpy())]
      genotype = Genotype(recurrent=gene, attention=att,concat=range(STEPS+1)[-CONCAT:])
      return genotype
