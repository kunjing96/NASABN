...
class MultiHeadAttn(nn.Module):
    def __init__(self, n_head, d_model, d_head, dropout=0, dropatt=0):
        ...
        self.q_net = nn.Linear(d_model, n_head * d_head, bias=False).cuda()
        self.kv_net = nn.Linear(d_model, 2 * n_head * d_head, bias=False).cuda()

        self.drop = nn.Dropout(dropout)
        self.dropatt = nn.Dropout(dropatt)
        self.o_net = nn.Linear(n_head * d_head, d_model, bias=False).cuda()
    ...
    def forward(self, h, attn_mask=None, mems=None):
        ...
        head_q = self.q_net(h.cuda())
        head_k, head_v = torch.chunk(self.kv_net(c.cuda()), 2, -1)
