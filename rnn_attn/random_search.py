...
if not args.continue_train:
    args.save = 'random_search-{}-{}'.format(args.save, time.strftime("%Y%m%d-%H%M%S"))
...
def geno(weights):
    def _parse(probs):
        ...
        for i in range(genotypes.STEPS):
            ...
            j = sorted(range(i + 1), key=lambda x: -max(W[x][k] for k in range(len(W[x])) if k != genotypes.PRIMITIVES.index('none')))[0]
            k_best = None
            for k in range(len(W[j])):
                if k != genotypes.PRIMITIVES.index('none'):
                    if k_best is None or W[j][k] > W[j][k_best]:
                        k_best = k
            gene.append((genotypes.PRIMITIVES[k_best], j))
            start = end
        return gene

    gene = _parse(F.softmax(torch.autograd.Variable(torch.Tensor(weights)), dim=-1).data.cpu().numpy())
    genotype = genotypes.Genotype(recurrent=gene, concat=range(genotypes.STEPS+1)[-genotypes.CONCAT:])
    return genotype
...
best_arch = None
for s in range(20):
    k = sum(i for i in range(1, genotypes.STEPS+1))
    weights = np.random.randn(k, len(genotypes.PRIMITIVES)) * 1e-3
    genotype = geno(weights)
    ...
    # Run on test data.
    test_loss = evaluate(val_data, eval_batch_size)
    logging.info('=' * 89)
    ...
    if best_test_loss is None or test_loss < best_test_loss:
        ...
        best_arch = genotype
        logging.info('-' * 89)
        logging.info('| sample arch {:3d} | test loss {:5.2f} | test ppl {:8.2f}'.format(s, best_test_loss, math.exp(best_test_loss)))
        logging.info('The best current genotype: {}'.format(genotype))
        logging.info('-' * 89)
