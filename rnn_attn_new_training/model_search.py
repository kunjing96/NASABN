...
class DARTSCellSearch(DARTSCell):
  ...
  def cell(self, x, h_prev, x_mask, h_mask, arch_mask):
    s0 = self._compute_init_state(x, h_prev, x_mask, h_mask)
    s0 = self.bn(s0)

    if arch_mask:
        weights_zeros = torch.zeros_like(self.weights)
        probs = torch.cat([weights_zeros[:,:1], F.softmax(self.weights[:,1:-1], dim=-1), weights_zeros[:,-1:]], -1)
    else:
...
class RNNModelSearch(RNNModel):
    ...
    '''def forward(self, input, hidden, return_h=False):
        ...
        raw_output = self.bn(raw_output.permute(1,2,0).contiguous())
        raw_output = raw_output.permute(2,0,1).contiguous()
        raw_outputs.append(raw_output)
    ...
    def _loss(self, hidden, input, target, arch_mask=False):
      log_prob, hidden_next = self(input, hidden, return_h=False, arch_mask=arch_mask)
